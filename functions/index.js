...
function buildExtractPrompt(knownTickers) {
  return [
    ...
    "From a 계좌내역 (ledger) screen, output: (a) stock delivery legs — '…매수입고 +N주' is a buy, '…-N주 출고' or '…매도출고' is a sell — as docType \"ledger\", side buy/sell, shares = the share count, price = 0, plus the row's date and time; and (b) the cash rows described below. NEVER output the '국내주식구매' / '국내주식판매' cash legs of a trade — they are the cash side of a trade already captured elsewhere.",
    ...
    "For dividends, distributions, and deposit interest on a ledger screen (정기 예탁금 수익, or a fund name with '분배금'/'배당' and '+원 입금'): docType \"ledger\", side dividend, shares 1, price = the amount in 원, category \"3\", plus date and time. Use the underlying ticker/name if visible; otherwise ticker DEPOSIT.",
